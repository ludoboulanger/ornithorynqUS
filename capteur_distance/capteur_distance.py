...
class DistanceSensor:
    ...
    def get_sim_distance(self, debug=False):
        distance = self.get_raw_distance(debug=debug)
        if distance is None or distance <= 0:
            return distance
        return self.apply_trendline(raw_distance=distance)
...
def sim_raw_single(sensor):
    ...
    obstacle_distance = sensor.get_raw_distance(debug=True)
    print_obstacle_distance(distance=obstacle_distance)
    print("\n -------------------------------------------------------------------------------------------------- \n")


def sim_corrected_single(sensor):
    print("\n ----------------------- Single object simulation - distance calculated with trendline ----------------------- \n")
    obstacle_distance = sensor.get_sim_distance(debug=True)
    print_obstacle_distance(distance=obstacle_distance)
    print("\n ------------------------------------------------------------------------------------------------------------- \n")
...
def sim_corrected_multiple(sensor, start_pos, start_angle, final_angle, angle_incr):
    ...
    for angle in range(start_angle, final_angle + angle_incr, angle_incr):
        ...
        obstacle_distance = sensor.get_sim_distance()
        print("\nCurrent vehicule angle around z axis : " + str(angle))
        print_obstacle_distance(distance=obstacle_distance)
    print("\n ------------------------------------------------------------------------------------------------------------- \n")
  
        
def print_obstacle_distance(distance):
    if distance is None or distance <= 0: 
        print("No obstacle found")
    else :
        print("Obstacle found at distance : " + str(distance) + " mm")
